...
export class BaseCrudService<T,K> {
  ...
  private readonly INVALID_REQUEST=-1;
  private readonly BOOK_NOT_FOUND=-2;
  private readonly BOOK_CODE_DUPLICATED=-12;
  private readonly BOOK_OPERATION_NOT_ALLOWED=-21;
  private readonly GENERIC_ERROR=-9999;
  ...
  updateOne(data: T | any): Observable<{ message: string, payload: T } | ResponseError> {
    return this.http
      .put<{ message: string, payload: Encryption }>(`${this.configurationLoader.getConfiguration().serverUrl}/${this.apiToken}?id=${data.id}`, data)
      .pipe(
        map((response: any) => {
          return response;
        }),
